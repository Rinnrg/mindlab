...
export default function Sintaks5Page() {
  ...
  useEffect(() => {
    ...
    async function fetchProyek() {
      try {
        const response = await fetch("/api/proyek?judul=Menganalisis dan Mengevaluasi Proses Pemecahan PBL")
        if (response.ok) {
    ...
    return (
      <div className="space-y-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-xl font-bold tracking-tight sm:text-2xl md:text-3xl">PBL - Problem Based Learning</h1>
          </div>
          {isGuruOrAdmin && (
            <Link href="/pbl/add/sintaks_5">
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl font-bold tracking-tight sm:text-2xl md:text-3xl">PBL - Problem Based Learning</h1>
          <p className="text-muted-foreground">Menganalisis dan Mengevaluasi Proses Pemecahan PBL</p>
        </div>
        {isGuruOrAdmin && proyek && (
          <Link href="/pbl/edit/sintaks_5">
        ...
        <Card className="md:col-span-2">
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <h3 className="font-semibold text-lg">Deskripsi PBL</h3>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <Button variant="secondary" className="w-full justify-start gap-2" asChild>
                <Link href={`/pbl/sintaks_5/submit?proyekId=${proyek.id}`}>
