...
export default function Sintaks4Page() {
  ...
  useEffect(() => {
    ...
    async function fetchProyek() {
      try {
        const response = await fetch("/api/proyek?judul=Mengembangkan dan Menyajikan Hasil Karya")
        if (response.ok) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl font-bold tracking-tight sm:text-2xl md:text-3xl">PBL - Problem Based Learning</h1>
          <p className="text-muted-foreground">Mengembangkan dan Menyajikan Hasil Karya</p>
        </div>
        {isGuruOrAdmin && proyek && (
          <Link href="/pbl/edit/sintaks_4">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <Button variant="secondary" className="w-full justify-start gap-2" asChild>
                <Link href={`/pbl/sintaks_4/submit?proyekId=${proyek.id}`}>
