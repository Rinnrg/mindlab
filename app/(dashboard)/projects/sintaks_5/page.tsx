...
export default function Sintaks5Page() {
  ...
  useEffect(() => {
    ...
    async function fetchProyek() {
      try {
        const response = await fetch("/api/proyek?judul=Menganalisis dan Mengevaluasi Proses Pemecahan Masalah")
        if (response.ok) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl font-bold tracking-tight sm:text-2xl md:text-3xl">PBL - Project Based Learning</h1>
          <p className="text-muted-foreground">Menganalisis dan Mengevaluasi Proses Pemecahan Masalah</p>
        </div>
